...
public class StationsControllerTest extends JsonControllerTest {
  ...
  public void setup() throws Exception {
    ...
    Arrays.asList("TVP1,TVP2,TVP3,Polsat".split(",")).forEach(s -> {
      final Station station = stationsRepository.save(new Station(s));
      stations.add(station);
    });

    Arrays.asList("One,Two,Three,Four,Five,Six".split(",")).forEach(s -> {
  ...
  public void testWrongContentType() throws Exception {
    mockMvc.perform(get("/stations").accept(unsupportedContent).contentType(unsupportedContent))
  ...
  public void testViewFields() throws Exception {
    for (Station station : stations) {
      mockMvc.perform(get(String.format("/stations/%d", station.getId())).accept(jsonContent).contentType(jsonContent))
          .andExpect(status().isOk())
          .andExpect(content().contentType(jsonContent))
  ...
  public void testDeleteNonExisting() throws Exception {
    ...
    mockMvc.perform(delete(String.format("/stations/%d/schedule", station.getId())).accept(jsonContent)
        .contentType(jsonContent)
        .content(serialize(body))).andExpect(status().isNotFound());
  }
  ...
  public void testDeleteExisting() throws Exception {
    ...
    mockMvc.perform(delete(String.format("/stations/%d/schedule", station.getId())).accept(jsonContent)
        .contentType(jsonContent)
        .content(serialize(body))).andExpect(status().isNoContent());
  ...
  public void testAddItem() throws Exception {
    ...
    mockMvc.perform(post(String.format("/stations/%d/schedule", station.getId())).accept(jsonContent)
        .contentType(jsonContent)
  ...
  public void testUpdateProgramRequiredParams() throws Exception {
    ...
    mockMvc.perform(patch(String.format("/stations/%d/schedule", station.getId())).accept(jsonContent)
        .contentType(jsonContent)
        .content(serialize(body))).andExpect(status().isBadRequest());
  }
  ...
  public void testUpdateProgramNotExistingStation() throws Exception {
    ...
    mockMvc.perform(patch(String.format("/stations/%d/schedule", -1)).accept(jsonContent)
        .contentType(jsonContent)
        .content(serialize(body))).andExpect(status().isNotFound());
  }
