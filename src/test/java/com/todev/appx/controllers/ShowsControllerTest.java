...
public class ShowsControllerTest extends JsonControllerTest {
  ...
  public void testWrongContentType() throws Exception {
    mockMvc.perform(get("/shows").accept(unsupportedContent).contentType(unsupportedContent))
  ...
  public void testAddItem() throws Exception {
    ...
    mockMvc.perform(post("/shows").accept(jsonContent).contentType(jsonContent).content(serialize(show)))
  ...
  public void testDetailsCorrect() throws Exception {
    for (Show show : shows) {
      mockMvc.perform(get(String.format("/shows/%d", show.getId())).accept(jsonContent).contentType(jsonContent))
          .andExpect(status().isOk())
          .andExpect(content().contentType(jsonContent))
  ...
  public void testUpdateRequiredParams() throws Exception {
    ...
    mockMvc.perform(patch(String.format("/shows/%d", show.getId())).accept(jsonContent)
        .contentType(jsonContent)
        .content(serialize(show))).andExpect(status().isBadRequest());
  }
  ...
  public void testDeleteNonExisting() throws Exception {
    mockMvc.perform(delete(String.format("/shows/%d", -1)).accept(jsonContent).contentType(jsonContent))
  ...
  public void testDeleteExisting() throws Exception {
    ...
    mockMvc.perform(delete(String.format("/shows/%d", show.getId())).accept(jsonContent).contentType(jsonContent))
        .andExpect(status().isNoContent());
